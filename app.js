...
export const createApp = (db) => {
  ...
  async function hasPendingReviews(studentID) {
    try {
      ...
      // Return whether there are pending reviews
      return completedReviews < totalTeammates;
    } catch (err) {
      console.error("Error calculating pending reviews:", err);
      throw err;
    }
  }
  ...
  app.get("/student-dashboard", async (req, res) => {
    ...
    try {
      // Use the utility function to check if there are pending reviews
      const pendingReviews = await hasPendingReviews(studentID);

      // Render the dashboard view with the computed data
      res.render("student-dashboard", {
        hasPendingReviews: pendingReviews,
  ...
  app.get("/assess-notification", async (req, res) => {
    try {
      ...
      res.render("assess-notification", {
        incompleteAssessments,
        hasPendingReviews: pendingReviews,
      });
    } catch (error) {
    ...
    `);
      ...
      const processedRows = rows.map((row) => ({
        ...
        conceptual_contribution:
          row.conceptual_contribution !== null
            ? row.conceptual_contribution
            : 0,
        practical_contribution:
          row.practical_contribution !== null ? row.practical_contribution : 0,
      ...
      res
        .status(500)
        .send("An error occurred while exporting the summary as CSV.");
    }
  });
...
=======
>>>>>>> a47cce8f240e7850b20ac1674f367d253046c8cd
...
>>>>>>> a47cce8f240e7850b20ac1674f367d253046c8cd
  ...
  app.post("/create-teams", upload.single("csvfile"), async (req, res) => {
    ...
    try {
                  ...
                  async (err, hash) => {
                    if (err) {
                      ...
                      await db.query(
                        ...
                        [
                          studentNameArray[i].toLowerCase(),
                          hash,
                          parseInt(teamIDarray[i]),
                        ]
                      );
                    }
                  }
      ...
      res.redirect(`/view-teams?instructorUsername=${instructorUsername}`);
    } catch (err) {
      console.log(err);
